...
def download_video(url, output_path, keep_video=False):
    base_path = output_path.replace('.wav', '')
    ydl_opts = {
        'format': 'bestvideo+bestaudio' if keep_video else 'bestaudio/best',
        'outtmpl': base_path,
        'postprocessors': [{
    ...
    if keep_video:
        ...
        video_path = f"{base_path}.{video_ext}"
        print(f"\nSaved video: {os.path.basename(video_path)}")
...
def main():
  ...
  Full options: python script.py "https://youtube.com/..." -of results --keep_video --detailed'''
    )
    ...
    temp_audio = os.path.join(args.output_folder, 'temp_audio')

    try:
        print(f"\n{' Starting Process ':-^50}")
        print("Downloading content...", end='', flush=True)
        actual_audio_path = download_video(args.url, temp_audio, args.keep_video)
